...
module.exports = (options = {}, context) => ({
  extendPageData($page) {
    ...
    const image_seo = (options = {}, frontmatter) => {
      const { default_image, default_image_type, default_image_width, default_image_height, default_image_alt } = options;

      const { image, image_type, image_width, image_height, image_alt } = frontmatter;

      let default_image_full_path;
      if (default_image) default_image_full_path = root_url + default_image;
      ...
      if (!is_duplicate_og('og:image') && !image) {
        ...
        Object.keys(push_og_image_default).forEach((property) => {
          if (!is_duplicate_og(property) && push_og_image_default[property]) push_og(property, push_og_image_default[property]);
        });

        if (!is_duplicate_tw('twitter:image') && !image)
          // case. image empty
      ...
      if (image) {
        ...
        if (!is_duplicate_ite('image')) frontmatter.meta.push(itemprop_template('image', image));
      }
    };
    ...
    const description_seo = () => {
      ...
      if (description) {
        if (!is_duplicate_og('og:description')) push_og('og:description', description);
        if (!is_duplicate_tw('twitter:description')) push_twit('twitter:description', description);
        if (!meta_description) frontmatter.meta.push({ name: 'description', content: description });
        if (!is_duplicate_ite('description')) frontmatter.meta.push(itemprop_template('description', description));
      ...
      if (meta_description) {
        ...
        if (!is_duplicate_tw('twitter:description')) push_twit('twitter:description', meta_description);
        if (!is_duplicate_ite('description')) frontmatter.meta.push(itemprop_template('description', meta_description));
      ...
      if (default_description) {
        if (!is_duplicate_og('og:description')) push_og('og:description', default_description);
        if (!is_duplicate_tw('twitter:description')) push_twit('twitter:description', default_description);
        if (!meta_description) frontmatter.meta.push({ name: 'description', content: default_description });
        if (!is_duplicate_ite('description')) frontmatter.meta.push(itemprop_template('description', default_description));
    ...
    const url_seo = () => {
      if (!is_duplicate_og('og:url')) push_og('og:url', full_url);
      if (!has_name(frontmatter, 'twitter:url')) push_twit('twitter:url', full_url);
    };
    ...
    const title_seo = () => {
      ...
      if (frontmatter.title) {
        if (!is_duplicate_og('og:title')) push_og('og:title', frontmatter.title);
        if (!is_duplicate_tw('twitter:title')) push_twit('twitter:title', frontmatter.title);
        if (!is_duplicate_ite('name')) frontmatter.meta.push(itemprop_template('name', frontmatter.title));
        return;
      }

      if (default_title) {
        if (!is_duplicate_og('og:title')) push_og('og:title', default_title);
        if (!is_duplicate_tw('twitter:title')) push_twit('twitter:title', default_title);
        if (!is_duplicate_ite('name')) frontmatter.meta.push(itemprop_template('name', default_title));
        return;
      }
    };
    ...
    const og_type_seo = () => {
      ...
      if (og_type) push_og('og:type', og_type);
      else push_og('og:type', default_og_type);
    };
    ...
    const twitter_card_seo = () => {
      ...
      if (!is_duplicate_tw('twitter:card') && twitter_card) push_twit('twitter:card', twitter_card);
      if (!is_duplicate_tw('twitter:card')) push_twit('twitter:card', default_twitter_card);
    };
    ...
    const site_name_seo = () => {
      ...
      if (!is_duplicate_og('og:site_name') && site_name) push_og('og:site_name', site_name);
      if (!is_duplicate_og('og:site_name') && default_site_name) push_og('og:site_name', default_site_name);
    };
...
const find_meta_description = (frontmatter) => {
  ...
  for (const elem of frontmatter.meta)
    if (elem.hasOwnProperty('name') && elem.name === 'name') {
      flag = true;
