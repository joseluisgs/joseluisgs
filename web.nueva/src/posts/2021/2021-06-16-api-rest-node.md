...
* Algunos [patrones de diseño](https://sourcemaking.com/design_patterns) conocida.
...
* [Dotenv](https://www.npmjs.com/package/dotenv). Para leer las variables de entorno del ficheor .env
...
## TypeScrip API REST
Ejemplo de un API REST realizada con TypeScript. Autenticación, CRUD, transferencia de ficheos. Modos de trabajo en: Memoria, MongoDB o MariaDB. Despliegue en Docker y test con Jest.
...
### Descripción
El proyecto consiste en que tengas un ejemplo de API REST pero realizada con TypeScript con el objetivo de mejorar con tipos tus desarrollos. Además propone el uso de ficheos, autenticación y autorización mediante JWT. Tiene tres modos de uso Memoria, con MongoDB (NoSQL) y MariaDB (SQL). Acceso desde: http://localhost:8000.
...
### EndPoints
Los Endpoints para conectarse y consumiur esta api rest, empiezan siempre por /api/vx/recurso, donde x es a versión de esta api, y recurso es el recurso a consumir, por ejemplo /api/v1/juegos desde http://localhost:8000.
...
| GET| /files | auth, grant('ADMIN') | Obtiene todos los ficheos. Solo Admin |
| GET | /files/id | auth | Obtiene datos del ficheo con el id indicado si pertenece al usuario/a |
| GET | /files/download/id | -- | Descarga el ficheo con el id indicado |
| POST | /files | auth | Añade el ficheo |
| PUT | /files/id | auth | Modifica el ficheo con el id indicado si pertenece al usuario/a |
| DELETE | /files/id | auth | Elimina el ficheo con el id indicado si pertenece al usuario/a |
